...
namespace Questao5.Application.Handlers
{
    public class MovimentarContaCommandHandler : IRequestHandler<MovimentarContaCommand, string>
    {
        ...
        public async Task<string> Handle(MovimentarContaCommand request, CancellationToken cancellationToken)
        {
            ...
            if (conta == null) throw new BusinessException("Conta corrente inválida.", ErrorCodes.INVALID_ACCOUNT);
            if (!conta.Ativo) throw new BusinessException("Conta corrente inativa.", ErrorCodes.INACTIVE_ACCOUNT);
            if (request.Valor <= 0) throw new BusinessException("O valor deve ser positivo.", ErrorCodes.INVALID_VALUE);
            if (request.TipoMovimento != "C" && request.TipoMovimento != "D")
                throw new BusinessException("Tipo de movimento inválido.", ErrorCodes.INVALID_TYPE);
